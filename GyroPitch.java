...
public class GyroPitch extends VBox{
	Label labelPitch;
	float min;
	float max;
	...
	public GyroPitch(float min, float max){		
		this.min = min;
		min = -360;
		this.max = max;
		max = 360;
		...
		this.getChildren().addAll(labelPitch, tfPitch, group);
	}
	
	public void newValue(double angle){
		tfPitch.setText(String.valueOf(angle));
